...
- **自動校正**: 高精度基準(SHT35 / BME680)と作業センサの差分を学習し `calib.json` に保存
...
| センサ I2C | AHT25 / BMP280 / BME680 / SHT35 | **HW I2C (Wire)** GPIO8/9 | OLED と別バス |

**I2C アドレス**: AHT20/25=0x38, SHT35(SHT3x)=0x44/0x45, BME280/BMP280=0x76/0x77, BME680=0x76/0x77
...
namespace calibauto {
  // 自動校正の基準（高精度）。SHT35(SHT3x) と/または BME680 が有れば校正
  // ※実機の基準は SHT35。SHT31 と同一 I2C アドレス/レジスタのため定数名は SHT31_ADDR のまま
  constexpr uint8_t SHT31_ADDR  = 0x45;   // SHT3x 共通(SHT30/31/35): 0x44 or 0x45
...
近接設置した高精度**基準センサ**（SHT35 / BME680）と、常用の**作業センサ**（AHT25 / BMP280）の
...
| 項目 | 作業センサ | 基準センサ | 適用先 |
|---|---|---|---|
| 気温 | AHT25 | **SHT35 優先**（無ければ BME680） | `g_calib.air` → AHT 温度に加算 |
| 湿度 | AHT25 | **SHT35 優先**（無ければ BME680） | `g_calib.humid` → AHT 湿度に加算 |
| 気圧 | BMP280 | BME680 | `g_calib.press` → BMP280 気圧に加算 |

> **温度基準は SHT35 単独**を使う（BME680 はガスヒータの自己発熱で温度が +0.5〜1℃ 高く出るため
> 温度基準に不適。BME680 は**気圧基準専用**）。SHT35 が無い構成でのみ BME680 温湿へフォールバック。

### 基準センサの精度（SHT35 / SHT31）

温度・湿度の基準に実際に使用しているのは **SHT35**（Sensirion SHT3x シリーズの最高精度グレード）です
（以前 README で「SHT31」と記載していたのは誤り）。SHT35 は SHT31 と**同一の I2C アドレス
(0x44/0x45)・レジスタ・通信プロトコル**で、**同じ Adafruit SHT31 ライブラリでそのまま動作**します
（コード変更・再ビルド不要）。

| センサ | 温度 精度 (typ) | 温度 精度 (max) | 湿度 精度 (typ) |
|---|---|---|---|
| SHT31 | ±0.2 °C | ±0.3 °C | ±2.0 %RH |
| **SHT35（実機使用）** | **±0.1 °C** | ±0.2 °C | ±1.5 %RH |

> 出典: Sensirion **SHT3x-DIS** データシート（typ=代表値）。SHT35 は SHT3x の中で温度・湿度とも
> 最も高精度なビン分けグレード。

**結論**: 基準が想定より高精度（温度で約 2 倍・湿度も向上）だったため、学習済みオフセットの信頼性は
**現行のまま問題なし、むしろ向上**しています。温度基準の誤差が小さいぶん、作業センサ(AHT25)の
校正はより正確な原点に収束しており、独り立ち後の実効精度も同等以上です。校正ロジック・目標許容誤差
（気温 ±0.5 °C / 気圧 ±2 hPa）は変更不要です。
...
高価な基準（SHT35 / BME680）を外し、AHT25 + BMP280 だけで運用する手順。
...
3. **取り外し**: 電源を切り、SHT35 と BME680 を物理的に外す（12V 混在環境での事故防止・高価パーツ回収）。
