...
static_assert(sizeof(HistRec) == 11, "HistRec は 11 バイトであること");
...
// ---- ライブ値 → レコード ----
inline HistRec encode(float water, float air, float humid, float press, int rpm, uint8_t fanOn) {
  HistRec r;
...
// ---- レコード → 物理値（デコード）----
struct HistVal { float water, air, humid, press; int rpm; uint8_t fanOn; };
inline HistVal decode(const HistRec& r) {
  return HistVal{ dec2(r.water), dec2(r.air), dec2(r.humid), decP(r.press), (int)r.rpm, r.fanOn };
}
...
bool append(uint32_t epoch, const HistRec& r);  // 1 サンプル追記
